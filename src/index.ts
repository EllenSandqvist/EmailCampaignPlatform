...
const cookieSecureOptions = process.env.NODE_ENV === 'production' ? true : false;

const cookieSameSiteOptions = process.env.NODE_ENV === 'production' ? 'none' : 'lax';
...
app.use(
  session({
    ...
    proxy: true,
    cookie: {
      secure: cookieSecureOptions,
      httpOnly: true,
      maxAge: 60 * 60 * 1000, // 1 hour
      sameSite: cookieSameSiteOptions,
    },
  })
);
app.use(
